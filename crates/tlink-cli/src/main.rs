...
fn classify(pkt: &[u8]) -> Seen<'_> {
    ...
    if fc.frame_type == tlink::dot11::TYPE_DATA {
        ...
        // predates it.
        let Some(d) = tlink::data::decapsulate(body80211) else {
...
fn print_frame(n: u64, rt: &Radiotap, d: &Dot11, af: &ActionFrame) {
    ...
    println!(
        "#{n}  {}  {}  v{}.{}  {} -> {}  {}  {}  tx_delay={}",
        tlink::action::subtype_name(af.fixed.subtype),
        freq,
                ...
                if let Some(s) = SyncParams::parse(t.value) {
                    println!(
                        ...
                        if s.is_self_master() {
                            ...
                            tlink::dot11::Mac(s.master).to_string()
                        },
                        s.tx_channel,
                    );
                }
...
fn run<T: pcap::Activated + ?Sized>(mut cap: pcap::Capture<T>, stats_only: bool) {
    ...
    while let Ok(pkt) = cap.next_packet() {
        total += 1;
        if let Some(rt) = tlink::radiotap::Radiotap::parse(pkt.data) {
            if rt.flags.is_none() {
                no_flags += 1;
            } else if rt.bad_fcs() {
                bad_fcs += 1;
                    ...
                    *by_tag.entry(t.tag).or_default() += 1;
                        ...
                        if let Some(sp) = SyncParams::parse(t.value) {
                            ...
                            *ap_align.entry(sp.ap_beacon_alignment_delta).or_default() += 1;
                            ...
                            let m = if sp.is_self_master() {
                                ...
                                tlink::dot11::Mac(sp.master).to_string()
                            };
                    ...
                    if t.tag == 33 {
                        if let Some(c) = SixGhzChannels::parse(t.value) {
                            let d = |p: Option<tlink::state::ClassChannel>| match p {
                    ...
                    if t.tag == 24 {
                        if let Some(e) = ElectionParamsV2::parse(t.value) {
                            ...
                            let m = if e.claims_mastership() {
                                ...
                                tlink::dot11::Mac(e.master).to_string()
                            };
    ...
    for (s, n) in &by_subtype {
        eprintln!("  {:<6} {n}", tlink::action::subtype_name(*s));
    }
    eprintln!("senders:");
    for (m, n) in &peers {
        eprintln!("  {m}  {n}");
    ...
    if !aw_periods.is_empty() {
        ...
        for (tu, n) in &aw_periods {
            eprintln!("  {tu} TU = {} us   in {n} frames", u32::from(*tu) * tlink::sync::TU_US);
        }
    }
    ...
    eprintln!("tags seen:");
    for (t, n) in &by_tag {
        eprintln!("  [{:>2}] {:<28} {n}", t, tlink::tlv::tag_name(*t));
    }
}
...
fn profile<T: pcap::Activated + ?Sized>(mut cap: pcap::Capture<T>) {
    ...
    while let Ok(pkt) = cap.next_packet() {
        ...
        match af.fixed.subtype {
            tlink::action::SUBTYPE_PSF => p.psf += 1,
            tlink::action::SUBTYPE_MIF => p.mif += 1,
            _ => {}
        }
        for t in af.tlvs() {
...
fn dump_tlv<T: pcap::Activated + ?Sized>(mut cap: pcap::Capture<T>, tag: u8, from: Option<&str>, window: Option<(u64, u64)>) {
    ...
    if seen.is_empty() {
        eprintln!("no tag {tag} ({}) in this capture{}", tlink::tlv::tag_name(tag),
            from.map(|m| format!(" from {m}")).unwrap_or_default());
        std::process::exit(1);
    }

    eprintln!("tag {tag} ({}): {} distinct value(s)", tlink::tlv::tag_name(tag), seen.len());
    ...
    for (i, (value, s)) in order.iter().enumerate() {
        ...
        println!("// tag {tag} ({}) -- {} bytes, seen {}x, first at frame {}",
            tlink::tlv::tag_name(tag), value.len(), s.count, s.first_frame);
...
fn coverage(args: &[String]) {
    use tlink::coverage::{is_decoded, of_tlv, Coverage};
    use std::collections::BTreeMap;
...
fn write_baseline(
    ...
    per_tag: &std::collections::BTreeMap<u8, (tlink::coverage::Coverage, u64)>,
) {
    let mut out = String::new();
    out.push_str("# tlink coverage floor -- the WORST-classified TLV of each tag.\n");
    out.push_str("#\n");
...
fn check_baseline(path: &str, floors: &std::collections::BTreeMap<u8, Floor>) -> i32 {
    ...
    for (tag, base, now) in &improved {
        println!(
            "IMPROVED  tag {tag:<3} {}  {}/{} -> {}/{}",
            tlink::tlv::tag_name(*tag), base.named, base.total, now.named, now.total
        );
    }
    for (tag, now) in &fresh {
        println!(
            "NEW TAG   tag {tag:<3} {}  {}/{} -- not in the baseline",
            tlink::tlv::tag_name(*tag), now.named, now.total
        );
    }
...
fn open_wonder(monitor: &str, channel: u8) -> Box<dyn tlink_hal::Radio> {
    use tlink_hal::TxParams;
    let mut w = match tlink_hal::wonder::Wonder::new(monitor) {
        Ok(w) => w,
        Err(e) => {
...
fn open_wonder(_monitor: &str, _channel: u8) -> Box<dyn tlink_hal::Radio> {
...
fn beacon(managed: &str, monitor: &str, channel: u8, secs: u64, psf_per_mif: u32, compete: bool, legacy: bool, metric: Option<u32>, per_window: u32, windows: Option<usize>, follow: bool, tenure: Option<u32>, datapath: Option<&str>, garbage: Option<&str>, version: Option<&str>, metric_floor: Option<u64>, wonder: bool, hop: bool) {
    use tlink_hal::{nl80211::Nl80211, Radio};
    ...
    let garbage = match garbage {
        None => None,
        Some(spec) => match tlink::beacon::Garbage::parse(spec) {
            Some(g) if g.any() => {
                eprintln!(
                    "  --garbage {spec}: {} (filling measured-constant bytes with 0x{:02x})",
                    g.describe(),
                    tlink::beacon::GARBAGE_BYTE
                );
                Some(g)
            }
            _ => {
    ...
    let stats = match tlink_session::run(radio.as_mut(), &cfg, &stop) {
        Ok(s) => s,
        Err(e) => {
        ...
        if stats.adopter_list.is_empty() {
            ...
            format!(
                ...
                stats
                    .adopter_list
                    .iter()
                    .map(|(m, n)| format!("{} x{n}", tlink::dot11::Mac(*m)))
                    .collect::<Vec<_>>()
                    .join(", ")
            )
        }
    ...
    if follow || hop {
        eprintln!(
            ...
            stats.anchors,
            stats.master.map(tlink::dot11::Mac),
            stats.phase_us,
            stats.spread_us,
...
/// A minimal stderr logger so `tlink-session`'s `log` output shows as it used to.
fn init_stderr_log() {
    use std::sync::Once;
...
fn follow<T: pcap::Activated + ?Sized>(mut cap: pcap::Capture<T>) {
    use tlink::election::ElectionParamsV2;
    use tlink::follow::Cluster;
    use tlink::sync::SyncParams;
    ...
    match cl.master {
        Some(m) => println!("master:  {} (metric {:?})", tlink::dot11::Mac(m), cl.master_metric),
        None => {
...
fn bytemap(args: &[String]) {
    ...
    for ((tag, len), (offs, n)) in &seen {
        let name = tlink::tlv::tag_name(*tag);
...
fn master_diff(args: &[String]) {
    ...
    for (mac, (m, f)) in &role_counts {
        ...
        println!("  {}  {m} / {f}{mark}", tlink::dot11::Mac(*mac));
    }
    println!();
...
fn topology(args: &[String]) {
    ...
    let fmt = |mac: [u8; 6]| tlink::dot11::Mac(mac).to_string();
...
fn tun(args: &[String]) {
    ...
    if let Some(mac) = args.get(2).and_then(|s| parse_mac(s)) {
        let a = tlink::data::link_local_from_mac(mac);
...
fn datapath(args: &[String]) {
    use tlink::data::{decapsulate, dst_mac_for_ipv6, is_ipv6_multicast, Encap, ETHERTYPE_IPV6};
    use tlink_hal::{poll::wait_readable, rawsock::RawSock, tun::Tun};
...
mod tests {
    ...
    /// The EUI-64 rule exists twice -- in `tlink::data` with the tests, and in
    /// `tlink_hal::tun` so the HAL need not depend on the protocol crate. A silent
    ...
    fn the_two_copies_of_the_eui64_rule_agree() {
        for mac in [
            ...
            assert_eq!(
                tlink::data::link_local_from_mac(mac),
                tlink_hal::tun::link_local(mac),
