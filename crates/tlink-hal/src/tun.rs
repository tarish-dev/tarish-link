...
//!
//! Without this, tlink is a control-plane library: it can join a cluster, hold a
...
//! (`tlink::data`), so an Ethernet header would be discarded immediately. TUN hands you
...
//! its AWDL MAC — see [`tlink::data::link_local_from_mac`] — so this interface must
...
/// cost is a 14-byte Ethernet header on every frame, which `tlink-session` strips inbound and
/// prepends outbound.
#[allow(dead_code)]
...
///
/// Duplicated from `tlink::data::link_local_from_mac` rather than depended on: the HAL
/// does not know about the protocol crate and should not start now. The rule is four lines
/// and `tlink`'s copy is the one with the tests — public here so a test can hold the two
