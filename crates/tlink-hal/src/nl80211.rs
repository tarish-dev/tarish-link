...
///
/// Duplicated here rather than depending on `tlink` because the HAL sits *below* the
/// protocol crate and must not depend upward. It is a dozen lines and the alternative is
/// a dependency cycle.
#[cfg(any(target_os = "linux", target_os = "android"))]
pub(crate) fn tlink_radiotap(b: &[u8]) -> Option<(Option<u64>, Option<u16>, Option<i8>)> {
    if b.len() < 8 || b[0] != 0 {
        return None;
    }
...
impl crate::Radio for Nl80211 {
    ...
    fn rx(&mut self, timeout_ms: u32) -> Result<Option<crate::RxFrame>> {
        #[cfg(any(target_os = "linux", target_os = "android"))]
        {
            ...
            let (tsf, freq, signal) = match tlink_radiotap(&buf) {
                Some(t) => t,
                None => (None, None, None),
            };
