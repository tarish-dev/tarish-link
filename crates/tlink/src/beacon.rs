...
/// `tlink::follow::DEFAULT_PRESENCE_MODE`.
///
...
pub struct Beacon {
    ...
    pub host: String,
    /// The regional social channel — 6, 44 or 149. See `tlink_hal::SOCIAL_CHANNELS`.
    ...
    /// Advertise a SINGLE-channel sequence (every slot on [`social_channel`]) instead of Apple's
    /// [6, 149] shape. Set when the radio cannot hop (`channel_lock`): a hop-less radio that claims
    /// ch6 in slot 8 makes a peer aim connection attempts at a channel we are never on, which they
    /// miss until one lands on our real channel — a multi-second connect delay (finding 127).
    /// libmosey on this same radio advertises exactly one channel; iOS reaches it in ~1.5s vs ~5s.
...
impl Beacon {
    ...
    pub fn schedule(&self) -> ChannelSequence {
        // A hop-less radio must advertise only the channel it is actually on (finding 127):
        // claiming [6, 149] sends peers to ch6 slots we never occupy, delaying the connect.
        if self.single_channel {
            return ChannelSequence::single(self.social_channel);
        }
