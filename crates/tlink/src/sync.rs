...
impl ChannelSequence {
    ...
    pub fn apple_shaped(social: u8, assoc: Option<u8>) -> ChannelSequence {
        Self::apple_shaped_impl(social, assoc)
    }

    /// Every slot on ONE channel — what a radio that cannot hop must advertise. Claiming the
    /// Apple-shaped [6, 149] (which hardcodes ch6 in slot 8) is a lie for a channel-locked radio:
    /// a peer aims connection attempts at the ch6 slots where we never are, they miss, and it
    /// retries until one lands on our real channel — a multi-second connect delay (finding 127).
    /// libmosey on this SAME hop-less radio advertises a single-channel sequence and iOS reaches
    /// it in ~1.5s vs our ~5s.
    pub fn single(chan: u8) -> ChannelSequence {
        let channels = vec![chan; 16];
        ChannelSequence {
    ...
    fn apple_shaped_impl(social: u8, assoc: Option<u8>) -> ChannelSequence {
