...
use tlink::action::{ActionFrame, SUBTYPE_MIF};
...
fn a_real_frame_from_a_real_apple_device_parses() {
    ...
    assert_eq!(tlvs.stop(), Some(tlink::tlv::Stop::Clean), "tags consume the region exactly");

    for required in [4u8, 5, 6, 18, 21] {
        assert!(
            tags.contains(&required),
            "a MIF carries tag {required} ({})",
            tlink::tlv::tag_name(required)
        );
    }
}
...
fn undocumented_tags_are_preserved_not_discarded() {
    ...
    for t in unknown {
        assert_eq!(tlink::tlv::tag_name(t), "unrecognised");
    }
}
...
fn the_two_channel_sequences_agree_on_occupancy_and_differ_on_channel() {
    ...
    let standalone = af
        .tlvs()
        .find(|t| t.tag == 18)
        .and_then(|t| tlink::sync::ChannelSequence::parse(t.value))
        .expect("tag 18 sequence");

    assert_eq!(embedded.encoding, tlink::sync::ChanEncoding::Legacy);
    assert_eq!(standalone.encoding, tlink::sync::ChanEncoding::OpClass);
...
fn sync_params() -> tlink::sync::SyncParams {
    let pkt = fixture_frame::FRAME;
    let rt = Radiotap::parse(pkt).unwrap();
    let b = rt.payload(pkt).unwrap();
    let d = Dot11::parse(b).unwrap();
    ...
    af.tlvs()
        .find(|t| t.tag == 4)
        .and_then(|t| tlink::sync::SyncParams::parse(t.value))
...
fn election_parameters_decode_and_the_counter_is_live() {
    ...
    let v1 = af
        .tlvs()
        .find(|t| t.tag == 5)
        .and_then(|t| tlink::election::ElectionParams::parse(t.value))
        .expect("tag 5 present in every frame observed");

    let v2 = af
        .tlvs()
        .find(|t| t.tag == 24)
        .and_then(|t| tlink::election::ElectionParamsV2::parse(t.value))
...
fn service_records_carry_the_airdrop_service_and_its_port() {
    ...
    for t in af.tlvs().filter(|t| t.tag == 2) {
        all.extend(tlink::service::records(t.value));
    }
    ...
    assert!(
        all.iter().any(|r| r.name().contains("_airdrop") || matches!(
            r, tlink::service::Record::Ptr { target, .. } if target.contains("_airdrop")
        )),
...
fn srv_records_are_big_endian_and_give_port_8770() {
    ...
    let ports: Vec<u16> = af
        .tlvs()
        .filter(|t| t.tag == 2)
        .flat_map(|t| tlink::service::records(t.value))
        .filter_map(|r| match r {
            tlink::service::Record::Srv { port, name, .. } if name.contains("_airdrop") => {
                Some(port)
            }
            _ => None,
        })
...
fn compressed_labels_expand_and_null_contributes_nothing() {
    use tlink::service::{compressed_label, decode_name};
...
fn data_path_state_discloses_the_ap_channel_but_zeroes_its_bssid() {
    use tlink::state::{flag, DataPathState};
...
fn the_undocumented_tags_carry_six_gigahertz_channels() {
    use tlink::state::{SixGhzChannels, SixGhzInfo};
