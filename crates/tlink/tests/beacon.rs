...
fn a_beacon_frame_parses_as_awdl_through_our_own_reader() {
    ...
    assert_eq!(d.bssid, Mac(tlink::action::BSSID));
...
fn the_default_metric_loses_to_a_real_apple_device() {
    use tlink::beacon::{METRIC_COMPETE, METRIC_DECLINE};
    use tlink::election::ElectionParamsV2;
...
fn the_advertised_psf_interval_is_the_one_we_would_pace_by() {
    use tlink::beacon::PSF_INTERVAL_TU;
    let b = Beacon::new(ADDR, 149, "QA");
    let f = b.mif(0);
    let af = ActionFrame::parse(&f[24..]).unwrap();
    let sync = SyncParams::parse(af.tlvs().find(|t| t.tag == 4).unwrap().value).unwrap();
...
fn garbage_reaches_the_encoded_tlvs() {
    use tlink::beacon::{Garbage, GARBAGE_BYTE};
...
fn t24_probe_addresses_the_read_u32_at_offset_zero() {
    let g = tlink::beacon::Garbage::parse("t24@0=01").expect("spec parses");
    ...
    let mut e = tlink::election::ElectionParamsV2::claiming([2, 0, 0, 0, 0, 1], 600, 7);
