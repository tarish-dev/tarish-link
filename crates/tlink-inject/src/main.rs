...
//!
//! Builds frames with `tlink` and transmits them through `tlink-hal`'s AF_PACKET path.
...
fn main() {
    ...
    if args.len() < 3 {
        eprintln!("usage: tlink-inject [wonder-up] <iface> <channel> [count] [psf-per-mif]");
    ...
    eprintln!("tlink-inject: {iface} ch{channel} as {addr:02x?}, {count} frames, 1 MIF per {psf_per_mif} PSF");

    let mut b = tlink::beacon::Beacon::new(addr, channel, "QA");
    b.metric = tlink::beacon::METRIC_COMPETE;

    let sock = match tlink_hal::rawsock::RawSock::open(iface) {
        Ok(s) => s,
        Err(e) => {
...
fn main() {
    eprintln!("tlink-inject needs Linux or Android (AF_PACKET injection)");
    std::process::exit(1);
}
